...
document.getElementById("resetBtn").onclick = async () => {
  if (await Swal.fire({ title: "Confirma?", text: "Isso apagará todos os registros!", icon: "warning", showCancelButton: true })) {
...
document.getElementById("exportPDF").onclick = async () => {
  ...
  const snapshot = await db.collection("registros").get();
  let total = 0;
  let content = `Relatório - ${mesAtual}\n\n`;
...
async function renderCalendar() {
  ...
  const dayOfWeek = dateMoment.format("dddd"); // Dia da semana em PT

  const cell = document.createElement("div");
  cell.classList.add("day");
  if (dailyTotals[dateStr]) cell.classList.add("has-value");

  // Mostra dia da semana + número
  cell.innerHTML = `
...
// Função pra capitalizar primeira letra (ex: 'segunda' -> 'Segunda')
...
async function renderGoal() {
  ...
  goalBox.innerHTML = `
    ...
    <button onclick="deletarMeta()" style="margin-top:10px; background:#f44336;">❌ Deletar Meta</button>
  `;
}
...
async function calcularTotalMes() {
  ...
  document.getElementById("monthlyTotal").innerHTML = `<h3>Total do mês: R$${total.toFixed(2)}</h3>`;
}
...
async function excluir(id) {
  if (await Swal.fire({ title: "Confirma exclusão?", icon: "warning", showCancelButton: true })) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🕒 Verifica automaticamente todo dia
  setInterval(() => {
...
function updateThemeIcon() {
  if (document.body.classList.contains("dark")) {
    themeBtn.textContent = "☀️ Alternar Tema";
  } else {
